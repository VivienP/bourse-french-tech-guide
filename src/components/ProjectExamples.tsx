  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      {projects.map((project, index) => {
        const { ref, isVisible } = useScrollAnimation(0.1, index * 200);
            ...
            <Card className="h-full hover:shadow-lg transition-all duration-300 hover:scale-105">
              <CardHeader>
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-12 h-12 ${project.color} rounded-lg flex items-center justify-center transition-transform duration-300 hover:scale-110`}>
                    <project.icon className="h-6 w-6 text-white" />
                  </div>
                  <Badge variant="outline" className="transition-colors duration-300 hover:bg-primary/10">{project.sector}</Badge>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-primary/10 rounded-lg text-center transition-all duration-300 hover:bg-primary/20">
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg text-center transition-all duration-300 hover:bg-gray-100">
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {project.depenses.map((depense, depenseIndex) => (
                      <li key={depenseIndex} className="text-sm text-gray-600 flex items-center transition-colors duration-300 hover:text-gray-800">
                        <div className="w-1.5 h-1.5 bg-primary rounded-full mr-2 flex-shrink-0 transition-all duration-300 hover:scale-125"></div>
                        <span>{depense}</span>
                      </li>
                    ))}
                  </ul>
