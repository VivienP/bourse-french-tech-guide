...
const FAQSection = () => {
  const faqData = [{
    ...
    answer: "Les grandes métropoles (Paris, Lyon, Lille, Marseille, Bordeaux, Grenoble) sont plus compétitives en raison du volume de candidatures. Les antennes régionales moins sollicitées sont généralement plus accessibles. Si cela fait sens pour votre entreprise, immatriculer sa société dans des régions peu sollicitées peut vous permettre d'augmenter significativement vos financements Bpifrance."
  }, {
  ...
  return <section id="faq" className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqData.map((faq, index) => <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger className="text-left text-slate-700">
                {faq.question}
              </AccordionTrigger>
