    ...
    <section className="relative pt-36 sm:pt-44 md:pt-48 pb-16 px-4 sm:px-6 lg:px-8 flex flex-col justify-center overflow-hidden" style={{ minHeight: '75vh' }}>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        <div className="text-center">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/60 backdrop-blur-md font-medium text-foreground mb-8 animate-fade-in border-primary/40 border-2 text-xs cursor-default">
            <Sparkles className="h-4 w-4 text-primary" />
            Subvention Innovation
          </div>

          <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold text-foreground mb-6 animate-fade-in tracking-tight">
            Bourse French Tech
          </h1>
          <p className="text-base sm:text-xl md:text-base text-muted-foreground mb-8 max-w-3xl mx-auto leading-relaxed animate-fade-in [animation-delay:200ms] opacity-0 [animation-fill-mode:forwards]">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16 animate-fade-in [animation-delay:400ms] opacity-0 [animation-fill-mode:forwards]">
            <Button size="lg" variant="outline" onClick={() => scrollToSection('definition')} className="border-secondary text-secondary hover:bg-secondary hover:text-secondary-foreground px-8 py-3 transition-all duration-200 rounded-xl text-sm">
              Découvrir le dispositif
            </Button>
            <Link to="/chat">
              <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-3 transition-all duration-200 rounded-xl shadow-md hover:shadow-lg text-sm">
